...
//     bool findWord(vector<vector<char>>& board, string& word, int start, int x, int y){
//         bool ans = false;
//         if (start >(word.length()-1)) return true;
        
// //         if (dp.find(word.substr(start, word.length()-start))!=dp.end()) return dp[word.substr(start, word.length()-start)];
...
//                     {
//                         board[r][c] = '0';
//                         ans = ans || findWord(board, word, start +1, r+1, c);
//                         ans = ans || findWord(board, word, start +1, r-1, c);
//                         ans = ans || findWord(board, word, start +1, r, c+1);
//                         ans = ans || findWord(board, word, start +1, r, c-1);
...
//                 // }
//                 board[x][y] = '0';
//                 ans = ans || findWord(board, word, start +1, x+1, y);
//                 ans = ans || findWord(board, word, start +1, x-1, y);
//                 ans = ans || findWord(board, word, start +1, x, y+1);
//                 ans = ans || findWord(board, word, start +1, x, y-1);
//                 board[x][y] = word[start];
//             }

//         }
//         // dp[word.substr(start, word.length()-start)] = ans;
//         return ans;
//     }
...
//             if (words[i].length()>(n*m)){
//                 continue;
//             }
//             if (findWord(board, words[i], 0, -1, -1)==true)
